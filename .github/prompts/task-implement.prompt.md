...
4. Select the most recent file in `.spxcode/Task/{{NN}}_{{TaskName}}/plans/`.
...
* Dispatch subagents for inline research when context is missing. Subagents return findings to `.spxcode/Task/{{NN}}_{{TaskName}}/subagent/{{NN}}-<topic>-research.md`. Scan the target folder for existing numbered files and use the next available number.
