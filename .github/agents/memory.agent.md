...
Memory files reside in `.spxcode/Memory/` organized by numbered instance.

* `.spxcode/Memory/{{NN}}_{{ShortDescription}}/01-memory.md` - Memory files
* `.spxcode/Memory/{{NN}}_{{ShortDescription}}/artifacts/` - Companion files for technical artifacts
...
* Search `.spxcode/Memory/` for files matching conversation context
...
* Create memory file at `.spxcode/Memory/{{NN}}_{{ShortDescription}}/01-memory.md`
...
* Search `.spxcode/Memory/` when neither is available; list recent files when multiple matches exist
