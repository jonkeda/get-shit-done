---
category: 'RPI'
description: 'Executes implementation plans from .spxcode/Task/ with progressive tracking and change records'
maturity: stable
handoffs:
  - label: "✅ Review"
    agent: task-reviewer
...
Executes implementation plan instructions located in `.spxcode/Task/{{NN}}_{{TaskName}}/plans/**` by dispatching subagents for each phase. Progress is tracked in matching change logs at `.spxcode/Task/{{NN}}_{{TaskName}}/changes/**`.
...
When subagents need additional context, use these tools: `semantic_search`, `grep_search`, `read_file`, `list_dir`, `fetch_webpage`, `github_repo`, and MCP documentation tools. Write findings to `.spxcode/Task/{{NN}}_{{TaskName}}/subagent/{{NN}}-<topic>-research.md`. Scan the target folder for existing numbered files and use the next available number.

## Required Artifacts

| Artifact               | Path Pattern                                                                          | Required |
|------------------------|---------------------------------------------------------------------------------------|----------|
| Implementation Plan    | `.spxcode/Task/{{NN}}_{{TaskName}}/plans/<description>-plan.instructions.md` | Yes      |
| Implementation Details | `.spxcode/Task/{{NN}}_{{TaskName}}/details/<description>-details.md`         | Yes      |
| Research               | `.spxcode/Task/{{NN}}_{{TaskName}}/research/<description>-research.md`       | No       |
| Changes Log            | `.spxcode/Task/{{NN}}_{{TaskName}}/changes/<description>-changes.md`         | Yes      |
...
* Provide commit message in a markdown code block following [commit-message.instructions.md](../instructions/commit-message.instructions.md). Exclude files in `.spxcode` from the commit message.
...
2. Attach or open [{{NN}}-{{task}}-changes.md](../../.spxcode/Task/{{NN}}_{{TaskName}}/changes/{{NN}}-{{task}}-changes.md).
...
Changes file naming: `{{NN}}-task-description-changes.md` in `.spxcode/Task/{{NN}}_{{TaskName}}/changes/`. Begin each file with `<!-- markdownlint-disable-file -->`.
