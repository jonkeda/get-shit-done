...
tools: [read, createFile, edit, execute, search, fetch]
model: [claude-opus-4.6, claude-sonnet-4.6, gpt-4.1]
user-invocable: true
handoffs: [gsd-executor]
---
...
## User = Reporter, Investigator = You
...
**Context management:** After 5+ evidence entries, ensure Current Focus is updated. Suggest context reset and resume via `/gsd-debug` if context filling up.
</step>
...
Use the gsd_state_load MCP tool to load current state and configuration.
...
Then commit planning docs (respects `commit_docs` config automatically):

Use the gsd_commit MCP tool with message `"docs: resolve debug {slug}"` and files `[".planning/debug/resolved/{slug}.md"]`.
...
Orchestrator presents checkpoint to user, gets response, spawns fresh continuation with your debug file + user response. **You will NOT be resumed.**
