...
Planning files reside in `.spxcode/Task/{{NN}}_{{TaskName}}/` at the workspace root unless the user specifies a different location.

* `.spxcode/Task/{{NN}}_{{TaskName}}/plans/` - Implementation plans (`{{NN}}-task-description-plan.instructions.md`)
* `.spxcode/Task/{{NN}}_{{TaskName}}/details/` - Implementation details (`{{NN}}-task-description-details.md`)
* `.spxcode/Task/{{NN}}_{{TaskName}}/research/` - Source research files (`{{NN}}-task-description-research.md`)
...
* Check for research files in `.spxcode/Task/{{NN}}_{{TaskName}}/research/` matching the task.
* When a research document contains a Decisions section, treat checked `[x]` selections as locked constraints. Do not re-evaluate alternatives or override these selections.
...
Have subagents write findings to `.spxcode/Task/{{NN}}_{{TaskName}}/subagent/{{NN}}-<topic>-research.md`.
...
* Write only to `.spxcode/Task/{{NN}}_{{TaskName}}/plans/`, `.spxcode/Task/{{NN}}_{{TaskName}}/details/`, and `.spxcode/Task/{{NN}}_{{TaskName}}/research/`.
...
Stored in `.spxcode/Task/{{NN}}_{{TaskName}}/plans/` with `-plan.instructions.md` suffix.
...
Stored in `.spxcode/Task/{{NN}}_{{TaskName}}/details/` with `-details.md` suffix.
...
version: '2.2.61'
---
applyTo: '.spxcode/Task/{{NN}}_{{TaskName}}/changes/{{NN}}-{{task_description}}-changes.md'
defaultEnabled: true
version: '2.2.61'
---
...
* [ ] Step 1.1: {{specific_action_1_1}}
  * Details: .spxcode/Task/{{NN}}_{{TaskName}}/details/{{NN}}-{{task_description}}-details.md (Lines {{line_start}}-{{line_end}})
...
2. Attach or open [{{NN}}-{{task}}-plan.instructions.md](.spxcode/Task/{{NN}}_{{TaskName}}/plans/{{NN}}-{{task}}-plan.instructions.md).
3. Start implementation by typing `/task-implement`.

## Resumption

When resuming planning work, assess existing artifacts in `.spxcode/Task/{{NN}}_{{TaskName}}/` and continue from where work stopped. Preserve completed work, fill gaps, update line number references, and verify cross-references remain accurate.
