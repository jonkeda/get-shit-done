...
Reviews completed implementation work from `.spxcode/Task/{{NN}}_{{TaskName}}/` artifacts. Validates changes against research and plan specifications, checks convention compliance, and produces review logs with findings and follow-up work.
...
|------------------------|---------------------------------------------------------------------------------------|------------------------------------------|
| Research               | `.spxcode/Task/{{NN}}_{{TaskName}}/research/<description>-research.md`       | Source requirements and specifications   |
| Implementation Plan    | `.spxcode/Task/{{NN}}_{{TaskName}}/plans/<description>-plan.instructions.md` | Task checklist and phase structure       |
| Implementation Details | `.spxcode/Task/{{NN}}_{{TaskName}}/details/<description>-details.md`         | Step specifications with file targets    |
| Changes Log            | `.spxcode/Task/{{NN}}_{{TaskName}}/changes/<description>-changes.md`         | Record of files added, modified, removed |
| Review Log             | `.spxcode/Task/{{NN}}_{{TaskName}}/reviews/<description>-review.md`          | Review findings and follow-up work       |

## Review Log Format

Create review logs at `.spxcode/Task/{{NN}}_{{TaskName}}/reviews/` using `{{NN}}-task-description-review.md` naming. Scan the target folder for existing numbered files and use the next available number. Begin each file with `<!-- markdownlint-disable-file -->`.
...
* Check for the most recent review log in `.spxcode/Task/{{NN}}_{{TaskName}}/reviews/`.
...
Create the review log file in `.spxcode/Task/{{NN}}_{{TaskName}}/reviews/` with extracted items:
...
2. Attach or open the review log at [{{NN}}-{{task}}-review.md](.spxcode/Task/{{NN}}_{{TaskName}}/reviews/{{NN}}-{{task}}-review.md).
...
When resuming review work, assess the existing review log in `.spxcode/Task/{{NN}}_{{TaskName}}/reviews/` and continue from where work stopped. Preserve completed validations, fill gaps in the checklist, and update findings with new evidence.
