...
Note: VS Code shows a validation warning for the `maturity:` field as it's not in VS Code's schema. This is expected; the field is required by the spxcode codebase for artifact lifecycle tracking. Ignore VS Code validation warnings for the `maturity:` attribute.
...
2. Attach or open `.spxcode/Task/01_MyTask/01-task-plan.md`.

<!-- Use markdown links for file references -->
2. Attach or open [01-task-plan.md](.spxcode/Task/01_MyTask/01-task-plan.md).
