...
Run three separate mapping passes, one per sub-repo, for maximum depth:

**Run 1 — Exact.Construction (the target app):**
```
/gsd-map-codebase Focus on the Exact-Construction/ sub-repo. This is the target .NET MAUI app — it's scaffolded but needs fixing. Document its current state, architecture, what's broken, and what patterns are already in place.
```

**Run 2 — Exact.Online (the architecture reference):**

When GSD detects existing `.planning/codebase/` files, choose **Update**.

```
/gsd-map-codebase Focus on the Exact-Online/ sub-repo. This is the reference .NET MAUI app whose architecture Exact.Construction must follow. Document its patterns (MVVM, DI, navigation, services, data layer) as the target architecture template.
```

**Run 3 — Bouw7 (the source apps to port):**

Again choose **Update** when prompted.

```
/gsd-map-codebase Focus on the Bouw7/ sub-repo. These are the native iOS and Android apps being ported to MAUI. Document the feature inventory, API contracts, business logic, and platform-specific patterns — this is the source functionality we need to replicate.
```
...
├── STRUCTURE.md          ← Directory structure & module map
├── CONVENTIONS.md        ← Coding conventions & patterns
├── TESTING.md            ← Test infrastructure & coverage
└── CONCERNS.md           ← Tech debt, risks & concerns
```

Each run updates/enriches these 7 documents with layered analysis from the focused sub-repo.
...
This switches all agents to Claude Opus 4.6 as primary model.
